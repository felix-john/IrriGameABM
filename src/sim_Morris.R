...
simulate <- function(param.set, parameter.names, no.repeated.sim, nl.obj, trace.progress, iter.length, function.name, fix.params, fix.param.names, ...) {
  ...
  for (i in 1:no.repeated.sim)
  {
    ...
    infrastructure.criterion <- cal.crit$infrastructure[c(3, 6, 10)]
    # inequality criterion
    inequality.criterion <- cal.crit$gini_ext[c(3, 6, 10)]
