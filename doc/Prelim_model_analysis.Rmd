...
Weight for different utility values $τ$	| `mu` | [0, 30]	| 0, 7.5, 15, 22.5, 30
Minimum utility to be satisfied $u_{min}$ | `umin` |  [0, 30]     | 0, 7.5, 15, 22.5, 30
Fraction of agents initially to be satisfied $δ$	| `psat` | [0, 1]	|	0, 0.25, 0.5, 0.75, 1
...
$\delta$ remains the most influential factor, yet $\tau$ and $\alpha$ have become more important. The effect of $\alpha$ is highly non-monotonic (presumably depending on its sign).
For inequality, $\delta$ and $\alpha$ have the largest negative impact. For $\alpha$ this seems implausible, because it means that the larger the aversion to exploit others is the higher will be the inequality in extraction. 
In general, the effects of parameters strongly depend on other parameters settings.
