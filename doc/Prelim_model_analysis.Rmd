...
```{r read CoV data SV}
CoV_data_SV <- read.csv("../data/output/CoV_socval-F-baseline.csv", header = T, skip = 6)
...
## Verification of utility function
...
+ Why are $\alpha$ and $\beta$ limited to 1? Is there a reason for that in the literature? Higher values would generate a stronger impact of social values.
...
+ Water flow is not included in utility function. Thus, potential returns on investment aren't considered (e.g. in the above example they don't see the use of investing). Should that be included somehow?
...
Charness & Rabin 2002: $$ \text{if } \pi_B \geq \pi_A: U_B = (1 - \rho - \theta q) \pi_B + (\rho + \theta q) \pi_A \\
 \text{if } \pi_B \leq \pi_A: U_B = (1 - \sigma - \theta q) \pi_B + (\sigma + \theta q) \pi_A $$ 
...
```
    + Implementing that would require to convert several procedures into reporter procedures (`calcwaterflow`) --> already started